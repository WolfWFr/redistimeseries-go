...
func TestCreateMultiRangeCmdArguments(t *testing.T) {
	...
	tests := []struct {
		...
		{"latest",
			args{0, 1, *(NewMultiRangeOptions().SetLatest(true)),
				[]string{"labels!="}},
			[]interface{}{"0", "1", "LATEST", "FILTER", "labels!="}},
		{"withlabels",
		...
		{"withlabels, aggregation, count, and empty",
			args{0, 1, *(NewMultiRangeOptions().SetAggregation(AvgAggregation, 60).SetWithLabels(true).SetCount(120).SetEmpty(true)),
				[]string{"labels!="}},
			[]interface{}{"0", "1", "AGGREGATION", AvgAggregation, "60", "COUNT", "120", "WITHLABELS", "EMPTY", "FILTER", "labels!="}},
		{"withlabels, aggregation, count, and bucket timestamp",
			args{0, 1, *(NewMultiRangeOptions().SetAggregation(AvgAggregation, 60).SetWithLabels(true).SetCount(120).SetBucketTimestamp(MidBucketTimestamp)),
				[]string{"labels!="}},
			[]interface{}{"0", "1", "AGGREGATION", AvgAggregation, "60", "COUNT", "120", "WITHLABELS", "BUCKETTIMESTAMP", MidBucketTimestamp, "FILTER", "labels!="}},
